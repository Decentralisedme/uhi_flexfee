...
contract VolFeesHook is BaseHook, BrevisApp, Ownable {
    ...
    using StateLibrary for IPoolManger;
    ...
    // Use getSlot0 to get current tick and current price
    function getCurrentTick(PoolKey calldata key) public returns (int24) {
	PoolId poolId = key.toId();
	(, int24 currentTick,,) = poolManager.getSlot0(poolId);
	return currentTick;
    }
    
    function getCurrentPrice(PoolKey calldata key) public returns (uint160) {
	PooId poolId = key.toId();
